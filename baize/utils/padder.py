...
def pad_sequence(sequence: list, max_length=None, pad_val=0, clip=True):
    """
    Pad several sequences `sequence` into the same length with the given pad value `pad_value`.
    If `max_length` is not set, the max_length will be set as the max length of the given sequence
    When `clip` is True, the sequence will be clipped into the `max_length`,
    otherwise the overlapped value will be kept (which may result in the lengths of sequneces becoming different).
